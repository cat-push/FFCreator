...
const creator = require("./node/cons");
...
class FFCreator extends FFCon {
   ...
   * @param {string} cover - the cover face image path
   * @public
   */
  setCover(cover) {
   ...
   * @param {string|object} args - the audio config
   * @public
   */
  addAudio(args) {
    if (!args) return;
  ...
  async start(delay = 25) {
    ...
    this.addRenderEvent();
    forEach(creator.children, child => child.start());
    this.renderer.start();
  }
