...
class FFNode extends FFBase {
   ...
   *
   * @constructor
   * @param {*&{type: string}} conf - FFNode related configuration items
  ...
  getWH() {
    ...
    if (width || height) {
      return [width, height];
    } else if (this.display.width || this.display.height) {
  ...
  addEffect(type, time, delay) {
    ...
    this.addAnimationsHistory.push(function(c){
      c.animations.addEffect(type, time, delay);
    })
  }

  addAnimate(animation) {
    this.addAnimationsHistory.push(function(c){
      c.animations.addAnimate(animation);})
  ...
  destroy() {
    ...
    this.display = null;
    this.parent = null;
    this.addAnimationsHistory = null
  }
