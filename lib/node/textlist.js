...
class FFTextlist extends FFNode {
    ...
    createAniNames() {
        const { effectname = "fadeInDown" } = this.conf
        return effectname
    }
    ...
    createEffectAndStart() {
        let curX = 0
        let curY = 0
        let { delay  = 0, pageTime = 3, pageEffect = "fadingOut", stopTime = -1 }  = this.conf
        let curTime = delay
            ...
            if (y + wh[1] > thisWh[1]) {
                ...
                if(curPageNum >= this.page ){
                    for (const t of curPage) {
                      t.addEffect(pageEffect, this.time, curTime - this.duration + stopTime)
                    }
                    break
                }
