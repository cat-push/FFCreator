...
class FFEmojiText extends FFContainer {
  ...
  createEmoji(text,width,height){
    let image = this.font.layout(text).glyphs[0].getImageForSize(Math.max(width ,height));
    let texture = Texture.fromImage(image.data);
    return new FFImage({
      image: texture,
      width: width,
      height: this.textwh[1],
      resetXY: true,
    })
  }
  ...
  createSingleLineNode(textlist){
    ...
    for (const data of textlist) {
      if(data.type === "text"){
        ...
        let imageWidth = charWidth*[...data.emoji].length
        let image_node = this.createEmoji(data.text,imageWidth,this.textwh[1])
  ...
  createMultiLineNode(textlist){
    ...
    for (const data of textlist) {
      if(data.type === "text"){
        ...
        let imageWidth = emojiWidth*[...data.emoji].length
        let image_node = this.createEmoji(data.text,imageWidth,this.textwh[1])
        this.addChild(image_node)
