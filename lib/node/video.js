...
const { Sprite, Texture, BaseTexture, Rectangle} = require('inkpaint');
...
class FFVideo extends FFImage {
  constructor(conf) {
    ...
    this.adaptive = conf.adaptive === undefined ? true : conf.adaptive;
    this.speed = conf.speed || 1
    this.endAnime = conf.endAnime
  ...
  async startHook() {
    if(this.endAnime && !this.loop){
      try {
        let info =  await this.getVideoInfo()
        let endTime = info.duration/1000
        let startTime = this.startTime === DEFAULT_TIME ? 0 : DateUtil.hmsToSeconds(this.startTime);
        this.addEffect(this.endAnime, 0, endTime+startTime)
      } catch (e) {
        FFLogger.error({ pos: 'FFVideo', error: e });
      }
    }
   ...
   */
  addTimelineCallback() {
    if(this.endAnime && this.animations.list.length === 1){
  ...
  drawCoverImage() {
    ...
    if (this.adaptive) {
      ...
      frame = this.materials.getSourceRect(width, height);
    }
  ...
  extractVideo() {
    return new Promise((resolve, reject) => {
      ...
      let opts = `-loglevel info -pix_fmt rgba -start_number 0 -vf fps=${fps},setpts=${1/this.speed}*PTS -qscale:v ${qscale}`.split(
        ' ',
      );
      ...
      if (this.adaptive) {
        const [width, height] = this.getWH();
        opts = opts.concat(["-s", `${width}*${height}`]);
      }
