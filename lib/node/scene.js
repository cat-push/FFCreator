...
class FFScene extends FFCon {
  ...
  async startHook(){
    for (const child of this.children) {
      await child.startHook();
    }
    let childEndTime = this.getListMaxStopTime(this.children)
    const {setAudioTime = false} = this.conf
    if(childEndTime < this.duration){
      childEndTime = this.duration
    }
    if(setAudioTime){
      let audioEndTime = await this.getAudioTime()
      childEndTime = min([childEndTime, audioEndTime])
    }
  ...
  async getAudioTime(){
    let endTime = 0
    for (const audio of this.parent.audios) {
      let info = await probe(audio.path);

      let audioEndTime = info.duration/1000
      if(audioEndTime > endTime){
